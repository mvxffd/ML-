...
VERSION="1.0"
SCRIPT_NAME="ML"
GITHUB_RAW_URL="https://gh.kejilion.pro/https://github.com/mvxffd/shell/raw/refs/heads/main/LM%E5%B7%A5%E5%85%B7%E7%AE%B1.sh"
...
verify_license() {
    ...
    echo -e "${fg_cyan}${bold}  🔐 首次验证${reset}"
...
menu_png_batch() {
    ...
    if [ ! -d "$SRC_DIR" ]; then
        ...
        echo -e " ${fg_yellow}💡 请在手机内部存储创建文件夹 1 并放入PNG图片${reset}"
...
# 子菜单：打开指定链接（优先 Via，其次 OPPO，最后默认）
...
main() {
    ...
    if [ $update_status -eq 2 ]; then
        echo -e "${fg_gray}  ─────────────────────────────────────────${reset}"
        echo -e " ${fg_yellow}发现新版本，是否立即更新？${reset}"
        echo -e "  ${fg_cyan}1${reset}  立即更新"
